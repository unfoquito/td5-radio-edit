...
La experimentación responde hasta qué tamaño de instancia llega cada algoritmo exacto y cuál de ellos, en qué implementación y en qué lenguaje, conviene usar en la práctica. La sección~\ref{sec:e1} mide hasta dónde llega cada algoritmo exacto, en instancias sintéticas y en prefijos de una grabación real, la~\ref{sec:e2} aísla el efecto de cada poda, la~\ref{sec:e3} verifica el modelo de tiempo y memoria de la programación dinámica, la~\ref{sec:e4} compara C++ con Python sobre el mismo algoritmo y la~\ref{sec:e5} aplica la programación dinámica a grabaciones reales frente a truncar y a muestrear.
